		...
		foreach($posts_arr as $key => $ppb_post)
		{
			...
			if(!empty($image_url[0]))
			{
				...
				switch($portfolio_type)
				{
					...
					default:
						$return_html.= '
						<img src="'.$small_image_url[0].'" alt="" class="portfolio_img"/> 	
				        <div class="mask">
			...
			foreach($images_arr as $key => $image)
			{
				if($items > 0 && $counter<=$items)
				{
					...
					if(isset($image_url[0]) && !empty($image_url[0]))
					{
						...
						<a class="img_frame" href="'.esc_url($full_image_url[0]).'">
				    		<img src="'.esc_url($image_url[0]).'" alt="" class="portfolio_img static"/>
				    	</a>
				    	</div>';
					}
