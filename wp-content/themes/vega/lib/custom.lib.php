...
{ 
    ...
    { 
        ...
        { 
            $pageLinks .= '<a href="'.esc_url($baseLink.$start).'page='.$i.'" class="slide">'.$i.'</a>'; 
        } 
    } 
    ...
    { 
        ...
        $next = ($currentPage + 1 > $totalPages) ? false : '<a href="'.esc_url($baseLink.$start).'page='.($currentPage + 1).'" class="slide">Next</a>'; 
         
        $prev = ($currentPage - 1 <= 0 ) ? false : '<a href="'.esc_url($baseLink.$start).'page='.($currentPage - 1).'" class="slide">Previous</a>'; 
    } 
...
'reply_text' => __('Reply', THEMEDOMAIN), 'login_text' => __('Login to Reply', THEMEDOMAIN), 'max_depth' => $args['max_depth']))) ?></p>
			<?php
				}
			?>
			...
			<?php
				...
				{
			?>
					<a href="<?php echo esc_url($comment->comment_author_url); ?>"><strong style="float:left;margin-top:1px"><?php echo $comment->comment_author; ?></strong></a>
			<?php
				}
				else
				{
			...
			<div class="comment_date"><?php echo date(THEMEDATEFORMAT.", ".THEMETIMEFORMAT, strtotime($comment->comment_date)); ?></div>
      		<?php comment_text() ?>
...
function pp_posts($sort = 'recent', $items = 3, $echo = TRUE, $bg_color = 'black' , $echo_title = TRUE) 
{
	...
	if(!empty($posts))
	{
		...
		$return_html.= '<ul class="posts blog '.esc_attr($bg_color).'_wrapper">';
			...
			foreach($posts as $post)
			{
				...
				{
					$return_html.= '<a href="'.esc_url(get_permalink($post->ID)).'"><img src="'.esc_url($image_thumb[0]).'" alt="" class="img_nofade frame"/></a>';
				}
				$return_html.= '<strong class="header"><a href="'.esc_url(get_permalink($post->ID)).'">'.$post->post_title.'</a></strong>';
				$return_html.= '<div class="post_detail">'.get_the_time(THEMEDATEFORMAT, $post->ID).'</div>';
			}	
...
function pp_get_the_excerpt($id=false) 
{
	 global $post;
